...
if __name__ == '__main__':
    ...
    product_embeddings = pd.read_pickle(os.path.join(path,'product_embeddings.pkl'))
    ...
    user_dep_stat = pd.read_pickle(os.path.join(path, 'user_department_products.pkl'))
    user_aisle_stat = pd.read_pickle(os.path.join(path, 'user_aisle_products.pkl'))
    ...
    my_train = pd.read_csv(os.path.join(path,'cleaned_train/valid_onr_0.csv'), usecols=['user_id', 'product_id']+my_features)
    my_test = pd.read_csv(os.path.join(path, 'cleaned_test/test.csv'), usecols=['user_id', 'product_id']+my_features)

    order_train = pd.merge(order_train, my_train, on=['user_id', 'product_id'], how='left')
    order_test = pd.merge(order_test, my_test, on=['user_id', 'product_id'], how='left')
    
    features.extend(my_features)
    ...
    result.to_pickle(os.path.join(path, 'prediction_arboretum_v4.pkl'))